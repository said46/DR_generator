...
def save_DR(fname, data):
    try:
        with open(DR_name, "w") as my_file:
            print(f'Saving the result into {DR_name}...')
            my_file.write(str(txt_out))
    except Exception as e:
        print(f'WARNING: Cannot save DR file: {str(e)}!')
...
for row in range(2, sheet.max_row + 1):
    ...
    if node != node_prev:
        if tag_counter != 1:
            txt_out += tail
            DR_name = f"output/DR_IMPORT_{node_prev}.txt"
            save_DR(DR_name, txt_out)
    ...
    temp = COUNT_IN_DR - ((COUNT_IN_DR - 1) // x_max) * x_max - 1
    x = x_ini + temp * x_delta    

    temp = (COUNT_IN_DR - 1) // x_max
    ...
    if tag_counter == number_of_blocks:
        txt_out += tail
        DR_name = f"output/DR_IMPORT_{node}.txt"
        save_DR(DR_name, txt_out)
